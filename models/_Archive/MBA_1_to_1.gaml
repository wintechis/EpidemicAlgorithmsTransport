...
species transporter parent: superclass schedules:[]{
	...
	reflex check_surroundings_for_model when:(agents_inside(my_cell.neighbors) of_species station){
		...
		station s <-  first(agents_inside(my_cell.neighbors) of_species station); //gets the first station that is next to a transporter
		
		do add_knowledge(s.location, s.accept_color); //Adds a station to the tranporter's model, updates entries
	...
	//whenever other transporters are near me - that is: at least one of my adjacent neighbors contains another transporter
	reflex exchange_knowledge_PUSH when:(!empty(agents_inside(my_cell.neighbors) where (string(type_of(each)) = "transporter"))) {		
		...
		//look for lacking knowledge, then PUSH it to neighbor
		
		//////////@@@@ TODO : implement finding out which knowledge lacks (on both sides..?) and then exchanging what is actually known [suggestion: if contradiction => delete?]
		
		ask neighbor_transporters{
			
			list<rgb> interested_in <- self.station_position.keys - myself.station_position.keys; //ignore keys that are already known
			
			loop col over: interested_in{
				add (myself.station_position at col) to: self.station_position at: col; //add previously unknown keys and values to my knowledge 
			}	
	...
	reflex exchange_knowledge when:(!empty(agents_inside(my_cell.neighbors) where (string(type_of(each)) = "transporter"))) {		
		...
		map<rgb, point> cumulated_knowledge <- station_position; //initalize with own knowledge
