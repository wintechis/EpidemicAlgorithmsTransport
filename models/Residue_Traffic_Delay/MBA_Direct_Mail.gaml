...
species transporter parent: superclass schedules:[]{
	...
	list<rgb> mail_updates <- []; //List of station updated that agent wants to send to other all other agents as direct mail. Entries have shape [rgb] 
	...
	//Take all updates and send them as "mail" to ALL other agents whenever I have to mail something
	reflex direct_mail when: (!empty(mail_updates)) {
		
		//Demers et al. take SOME of the other databases; we send (PUSH) agents model to all other agents
		ask transporter{
						
			loop col over: myself.mail_updates {
				
				total_traffic <- total_traffic + 1; //sent update mail to other agent
						
				//if mail content is more up to date than receiver knowledge --> apply
				if(myself.timestamps at col > self.timestamps at col){ 
					...
					//save delay for this updated / new fact
					if(knowledge = true){
						do update_delay; 	
					}
					
					//Check if receiver had updated queued in its mail_updates and if so remove it to save unnecessary messages
					remove col from: self.mail_updates;
	...
	action add_station_to_model(station stat){
		...
		if(!(agent_model contains_key stat.accept_color)) //if no observation has been made before
		{
			...
			add stat.accept_color to: mail_updates; //send update mail to other agents
...
experiment MBA_Direct_Mail type: gui {
...
experiment Performance type: batch until: (cycle >= 5000) repeat: 20 autorun: true keep_seed: true{ 
    ...
    ask simulations {
    	...
    	save [int(self), disturbance_cycles, self.cycle, self.total_delivered, mean_cyc_to_deliver]
           to: "simulation_results/performance/DM_"+ experiment.name +"_"+ string(width)+".csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
...
experiment Knowledge type: batch until: (cycle >= 5000) repeat: 20 autorun: true keep_seed: true{ 
    ...
    ask simulations {
    	...
    	save [int(self), disturbance_cycles, self.cycle, mean(self.residue), avg_traffic, mean(t_avgs)]
           to: "simulation_results/knowledge/DM_"+ experiment.name +"_"+ string(width)+".csv" type: "csv" rewrite: false header: true; 
    	}       
	}		
}
